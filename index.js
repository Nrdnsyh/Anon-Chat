...
const mulai = async (sesi, conn = new WAConnection()) => {
     ...
     console.log(chalk.greenBright('[ WhatsApp ]  ') + moment(new Date()).format('HH:mm:ss DD/MM/YYYY') + ' ' + chalk.magentaBright('Maximum Matches In ' + settings.maxMatch + ' Seconds'))
     conn.on('qr', async qr => {
          status.stop()
          await delay(1000)
     ...
     conn.on('contacts-received', async () => {
          ...
          starting.stop()
          if (!IsShouldmessage) {
               console.log(chalk.greenBright('[ WhatsApp ]  ') + moment(new Date()).format('HH:mm:ss DD/MM/YYYY') + ' ' + chalk.magentaBright('Server Ready ✓'))
          } else {
               reconnect.stop()
          }
          reconnect.stop()
     ...
     conn.on('open', (json) => {
          ...
          starting.start()
          if (ConnectingWs) {
               console.log(chalk.greenBright('[ WhatsApp ]  ') + moment(new Date()).format('HH:mm:ss DD/MM/YYYY') + ' ' + chalk.cyanBright('Connected as ' + json.user.name))
               console.log(chalk.greenBright('[ WhatsApp ]  ') + moment(new Date()).format('HH:mm:ss DD/MM/YYYY') + ' ' + chalk.yellow(json.user.phone.device_manufacturer + ' Phone Detected'))
          } else {
               reconnect.stop()
               console.log(chalk.greenBright('[ WhatsApp ]  ') + moment(new Date()).format('HH:mm:ss DD/MM/YYYY') + ' ' + chalk.greenBright('Connected Back'))
               clientsNow.forEach((client) => {
                    if (!isClientLog) return
                    client.send(chalk.greenBright('[ WhatsApp ]  ') + moment(new Date()).format('HH:mm:ss DD/MM/YYYY') + chalk.greenBright(` Connected Back`))
               })
          }
     })
     ...
     const server = express()
          ...
          .listen(PORT, () => console.log(chalk.greenBright('[ WhatsApp ]  ') + moment(new Date()).format('HH:mm:ss DD/MM/YYYY') + ' ' + chalk.blueBright('Socket Ready On Port ' + PORT)));
     ...
     wss.on('connection', async (ws) => {
          ...
          console.log(chalk.greenBright('[ WhatsApp ]  ') + moment(new Date()).format('HH:mm:ss DD/MM/YYYY') + ' ' + chalk.bgBlueBright('Client ' + userID + ' Connected '))
          const statusConnected = chalk.greenBright('\nConnected client id : ' + clientsNow.length)
          ws.send(statusConnected)
          clientsNow.forEach((client) => {
               client.send(chalk.greenBright('[ WhatsApp ]  ') + moment(new Date()).format('HH:mm:ss DD/MM/YYYY') + ' ' + chalk.bgBlueBright('Client ' + userID + ' Connected '))
          });
          ...
          ws.on('close', () => {
               console.log(chalk.greenBright('[ WhatsApp ]  ') + moment(new Date()).format('HH:mm:ss DD/MM/YYYY') + ' ' + chalk.bgRedBright('Client Disconnected'))
               clientsNow.forEach((client) => {
                    client.send(chalk.greenBright('[ WhatsApp ]  ') + moment(new Date()).format('HH:mm:ss DD/MM/YYYY') + ' ' + chalk.bgRedBright('Client Disconnected'))
               });
          });
     });
     ...
     conn.on('ws-close', () => {
          ...
          clientsNow.forEach((client) => {
               if (!isClientLog) return
               client.send(chalk.greenBright('[ WhatsApp ]  ') + moment(new Date()).format('HH:mm:ss DD/MM/YYYY') + chalk.redBright(` Connection close, reconnect..`))
          })
          ConnectingWs = false
     })
     ...
     conn.on('close', async ({ reason, isReconnecting }) => {
          if (!isReconnecting) {
               console.log(chalk.redBright('[ WhatsApp ]  ') + chalk.yellow('Connect To Phone Rejected and Shutting Down.'))
               reconnect.stop()
               process.exit(1)
          }
     })
